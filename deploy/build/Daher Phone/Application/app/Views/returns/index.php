<?php
/**
 * Returns list. Expects: $pg, $filters
 */
?>
<div class="page-heading">
  ...
  <a href="<?= url('returns/create') ?>" class="btn btn-primary btn-sm">
    <i class="bi bi-arrow-counterclockwise me-1"></i>New return
  </a>
</div>
...
<div class="card">
  <div class="card-header">
    <form class="filters-bar" method="get" action="index.php">
      <input type="hidden" name="r" value="returns/index">
      <div>
        ...
        <input type="search" class="form-control form-control-sm" name="q" style="min-width:220px"
               placeholder="Return #, invoice # or customer…" value="<?= e($filters['q']) ?>">
      </div>
      <div>
      ...
      <div class="d-flex gap-2">
        ...
        <a class="btn btn-outline-secondary btn-sm" href="<?= url('returns/index') ?>">Reset</a>
      </div>
    </form>
  </div>
  ...
  <div class="table-responsive">
    <table class="table table-hover table-sticky align-middle mb-0">
      ...
      <tbody>
        <?php if ($pg['rows'] === []): ?>
        <tr><td colspan="7">
          <div class="empty-state"><i class="bi bi-arrow-counterclockwise"></i>No returns recorded.</div>
        </td></tr>
        <?php endif; ?>
        <?php foreach ($pg['rows'] as $r): ?>
        <tr>
          <td><a class="data fw-semibold" href="<?= url('returns/show', ['id' => $r['id']]) ?>"><?= e($r['return_no']) ?></a></td>
          ...
          <td class="num fw-semibold"><?= e(money($r['total_value'])) ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
